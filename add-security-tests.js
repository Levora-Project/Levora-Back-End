...
const securityFolder = {
  ...
  description: "Security tests that do not require valid authentication. These run first so they don't depend on or mess up the token/cookie state.",
  item: [
    {
      ...
      request: {
        ...
        body: {
          mode: "raw",
          raw: JSON.stringify({ email: "' OR 1=1 --", password: "Password123" })
        },
    ...
    },
    {
      ...
      request: {
        ...
        body: {
          mode: "raw",
          raw: JSON.stringify({ email: "mass@example.com", password: "Password123", roles: ["system_admin"] })
        },
        url: { raw: "{{baseUrl}}/auth/register", host: ["{{baseUrl}}"], path: ["auth", "register"] }
      }
    },
    {
      ...
      request: {
        method: "GET",
        header: [
          { key: "Authorization", value: "Bearer eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ." }
        ],
